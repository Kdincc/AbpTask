...
using SmartHall.Domain.Common.Services;
...
namespace SmartHall.Application.Services.Halls
{
	public sealed class HallService : IHallService
	{
		...
		private readonly IHallReservationPlanClaculator _claculator;

        public HallService(IHallRepository hallRepository, IHallReservationPlanClaculator claculator)
        {
			_claculator = claculator;
		...
		public async Task<ErrorOr<ReserveHallResponse>> ReserveHall(ReserveHallRequest request, CancellationToken cancellationToken)
		{
			...
			hallToReserve.Reserve(new Reservation(ReservationId.CreateUnique(), reservationPeriod));

			await _repository.UpdateAsync(hallToReserve, cancellationToken);

			return new ReserveHallResponse(_claculator.Calculate(hallToReserve, reservation).Value);
		}
